...
- **НИКОГДА не запускай `npm run dev` или `npm run start` в этом проекте** - предполагай что сервер уже запущен
- **Используй порт 4001 для тестирования фронтенда** - это порт на котором работает приложение
